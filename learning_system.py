...
class LearningSystem:
    ...
    def learn_from_reassignment(self, ticket_id: str, original_team: str, new_team: str, 
                                 original_intent: str = None, reason: str = None):
        """
    ...
    def get_learned_keywords_for_intent(self, intent: str) -> List[str]:
        ...
        keywords = [
            p['pattern_value'] 
            for p in patterns 
            if p['pattern_key'] == intent and p['confidence'] >= self.min_pattern_confidence
        ]
        
        # Sort by usage count
        keyword_usage = {p['pattern_value']: p['usage_count'] for p in patterns if p['pattern_key'] == intent}
    ...
    def get_learned_team_for_intent(self, intent: str) -> Optional[str]:
        ...
        matching_patterns = [
            p for p in patterns 
            if p['pattern_value'] == intent and p['confidence'] >= self.min_pattern_confidence
        ]
    ...
    def get_learning_stats(self) -> Dict:
        ...
        return {
            ...
            'reassignments': sum(1 for f in feedback if f['feedback_type'] == 'reassignment'),
