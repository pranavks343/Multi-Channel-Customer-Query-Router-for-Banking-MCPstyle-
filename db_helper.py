...
def view_tickets_by_team():
    ...
    cursor.execute("SELECT * FROM tickets WHERE assigned_team = ? ORDER BY created_at DESC", (team_name,))
    tickets = cursor.fetchall()
...
def view_specific_ticket():
    ...
    cursor.execute("SELECT * FROM routing_log WHERE ticket_id = ? ORDER BY timestamp", (ticket_id,))
