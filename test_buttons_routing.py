    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"  ✓ Assigned Team: {result.get('routing', {}).get('final_team')}")
                print(f"  ✓ Confidence: {result.get('classification', {}).get('confidence', 0) * 100:.0f}%")
                if result.get('response'):
                    print(f"  ✓ Auto-response generated: {len(result.get('response'))} chars")
                return True
            else:
                print(f"  ❌ API returned error: {data}")
                return False
