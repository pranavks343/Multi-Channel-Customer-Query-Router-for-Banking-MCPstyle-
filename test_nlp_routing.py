...
def test_nlp_routing():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(
            f"  Match: {'✅' if classification['intent'] == test_case['expected_intent'] else '❌'}"
        )

        print(f"\n✓ Urgency: {classification['urgency']}")
        print(f"  Expected: {test_case['expected_urgency']}")
        print(
            f"  Match: {'✅' if classification['urgency'] == test_case['expected_urgency'] else '❌'}"
        )
        ...
        results.append(
            {
                "test": test_case["name"],
                "intent_match": classification["intent"]
                == test_case["expected_intent"],
                "urgency_match": classification["urgency"]
    ...
    print(
        f"Intent Accuracy: {intent_matches}/{len(results)} ({intent_matches/len(results)*100:.1f}%)"
    )
    print(
        f"Urgency Accuracy: {urgency_matches}/{len(results)} ({urgency_matches/len(results)*100:.1f}%)"
    )
    print(
