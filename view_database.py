...
def view_database(db_path="query_router.db"):
    ...
    try:
        ...
        if log_count > 0:
            ...
            print(f"\n  Recent Events:")
            for log in logs:
                print(f"    • {log['event_type']} - {log['ticket_id']} - {log['timestamp']}")
...
def show_indices(db_path="query_router.db"):
    ...
    try:
        ...
        if not indices:
            ...
            print("  (SQLite automatically creates indices for PRIMARY KEY and UNIQUE constraints)")
            print()
        else:
            current_table = None
            for index_name, table_name, index_sql in indices:
                ...
                if index_sql:
                    ...
                    print(f"    Type: Auto-generated (PRIMARY KEY or UNIQUE constraint)")
                print()
        ...
        for table in tables:
            ...
            if table_indices:
                for idx_info in table_indices:
                    ...
                    if index_columns:
                        cols = [col[2] for col in index_columns]  # Column name is at index 2
