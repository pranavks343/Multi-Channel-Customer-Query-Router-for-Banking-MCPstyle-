...
class IntentClassifier:
    def __init__(self, learning_system=None):
        ...
        self.intent_categories = {
            "kyc_verification": [
                "account verification", "identity check", "kyc", "document upload",
                "verification stuck", "pending verification", "id verification"
            ],
            "technical_support": [
                "api error", "integration", "webhook", "error code", "technical issue",
                "500 error", "403 error", "404 error", "timeout", "rate limit", "sdk"
            ],
            "billing_finance": [
                "invoice", "billing", "charge", "payment", "refund", "subscription",
                "pricing", "cost", "fee", "balance", "transaction"
            ],
            "compliance_regulatory": [
                "compliance", "gdpr", "pci dss", "soc 2", "audit", "regulation",
                "data protection", "privacy", "security certificate", "certification"
            ],
            "sales_inquiry": [
                "demo", "pricing plan", "enterprise", "partnership", "white label",
                "new customer", "signup", "trial", "features"
            ],
            "general_support": [
                "help", "how to", "question", "information", "documentation", "guide"
            ]
        }
...
"""
    ...
    def _parse_gemini_response(self, response_text: str) -> Dict:
        ...
        try:
            ...
            print(f"Response text: {response_text[:500]}...")  # Truncate for readability
    ...
    def _fallback_classification(self, text: str) -> Dict:
        ...
        dates = re.findall(r'\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{1,2}', text_lower)
        key_entities.extend(dates)
        
        # Detect sentiment
        negative_words = ["error", "failed", "stuck", "problem", "issue", "dispute", "wrong", "incorrect", "can't", "unable"]
        urgent_words = ["urgent", "critical", "emergency", "immediately", "asap", "blocked", "down"]
        ...
        if any(word in text_lower for word in ["dispute", "discrepancy", "wrong charge", "incorrect billing", "billing error"]):
        ...
        for intent, keywords in self.intent_categories.items():
            ...
            if intent == "kyc_verification":
                if re.search(r'\b(?:verify|verification|kyc|document)\s+(?:stuck|pending|failed|issue)', text_lower):
                    score += 3
            elif intent == "technical_support":
                if re.search(r'\b(?:api|webhook|integration|sdk)\s+(?:error|failing|not working)', text_lower):
                    score += 3
            elif intent == "billing_finance":
                if re.search(r'\b(?:invoice|payment|charge|refund|billing)\s+(?:question|issue|problem)', text_lower):
                    score += 2
        ...
        reasoning_parts.append(f"Sentiment: {sentiment}, Urgency determined by context analysis")
    ...
    def _load_learned_keywords(self):
        ...
        for intent in self.intent_categories.keys():
            learned_keywords = self.learning_system.get_learned_keywords_for_intent(intent)
            # Merge learned keywords with existing ones
            existing_keywords = set(self.intent_categories[intent])
            new_keywords = [kw for kw in learned_keywords if kw not in existing_keywords]
            self.intent_categories[intent].extend(new_keywords[:10])  # Add top 10 learned keywords
