...
def test_email_classification():
    ...
    for i, email in enumerate(test_emails, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  Key Entities: {', '.join(classification.get('key_entities', []))}")
                ...
                if category_match and urgency_match:
                    ...
                    if not category_match:
                        print(f"   Expected category: {email['expected_category']}, Got: {actual_category}")
                    if not urgency_match:
                        print(f"   Expected urgency: {email['expected_urgency']}, Got: {actual_urgency}")
    ...
    for result in results:
        ...
        if result.get('success'):
            print(f"   → Category: {result.get('category')}, Urgency: {result.get('urgency')}")
