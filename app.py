...
from sample_data import get_sample_queries, get_canned_responses
import json
import time
    ...
    try:
        ...
        return jsonify({
            ...
            "result": {
                ...
                "classification": {
                    "intent": result['classification']['intent'],
                    "category": result['classification']['intent'],  # Alias for clarity
                    "urgency": result['classification']['urgency'],
                    "confidence": result['classification']['confidence'],
                    "reasoning": result['classification'].get('reasoning', ''),
                    "key_entities": result['classification'].get('key_entities', []),
...
def update_ticket(ticket_id):
    ...
    try:
        ...
        allowed_fields = ['status', 'assigned_team', 'urgency', 'intent', 'message', 'subject', 'sender', 'response']
        updates = {}
        ...
        for field, value in updates.items():
            if field == 'metadata':
                ...
                import json
                stored_metadata = json.loads(updated_ticket.get('metadata', '{}') or '{}')
                expected_metadata = json.loads(value) if isinstance(value, str) else value
