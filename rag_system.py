...
class RAGSystem:
    def __init__(self, persist_directory: str = "./chroma_db"):
        ...
        print(
            f"Initialized RAG system with {len(self.vector_store)} documentation pages loaded from 'pages' folder"
        )
    ...
    def _parse_html_page(self, html_file_path: str) -> Dict:
        ...
        try:
            ...
            overview_section = soup.find(
                "h2", string=re.compile("Overview", re.IGNORECASE)
            )
            overview = ""
            if overview_section:
            ...
            keywords_section = soup.find(
                "h2", string=re.compile("Keywords", re.IGNORECASE)
            )
            keywords = []
            if keywords_section:
                keywords_div = keywords_section.find_next_sibling(
                    "div", class_="keywords"
                )
                if keywords_div:
            ...
            # Extract use cases
            use_cases_section = soup.find(
                "h2", string=re.compile("Use Cases", re.IGNORECASE)
            )
            use_cases = []
            if use_cases_section:
                ...
                if use_cases_ul:
                    use_cases = [
                        li.get_text(strip=True) for li in use_cases_ul.find_all("li")
                    ]

            # Extract common scenarios
            scenarios_section = soup.find(
                "h2", string=re.compile("Common Scenarios", re.IGNORECASE)
            )
            scenarios = []
            if scenarios_section:
                ...
                if scenarios_ul:
                    scenarios = [
                        li.get_text(strip=True) for li in scenarios_ul.find_all("li")
                    ]

            # Extract template response
            template_section = soup.find(
                "h2", string=re.compile("Response Template", re.IGNORECASE)
            )
            template_response = ""
            if template_section:
                template_box = template_section.find_next_sibling(
                    "div", class_="template-box"
                )
                if template_box:
                    template_p = template_box.find("p")
                    template_response = (
                        template_p.get_text(strip=True) if template_p else ""
                    )

            # Extract response approach
            approach_section = soup.find(
                "h2", string=re.compile("Response Approach", re.IGNORECASE)
            )
            approach_steps = []
            if approach_section:
                ...
                if approach_ol:
                    approach_steps = [
                        li.get_text(strip=True) for li in approach_ol.find_all("li")
                    ]
    ...
    def _build_vector_store(self):
        ...
        if not pages_dir.exists():
            print(
                f"Warning: Pages directory not found at {pages_dir}. Falling back to canned responses."
            )
        ...
        if not html_files:
            print(
                "Warning: No HTML pages found in pages folder. Falling back to canned responses."
            )
            return self._build_fallback_vector_store()
    ...
    def retrieve_response(
        self, query: str, intent: str = None, n_results: int = 3
    ) -> List[Dict]:
        """
    ...
    def generate_response(
        ...
        try:
            ...
            generated_response = self._format_response(
                generated_response, customer_name
            )
...
"""
    ...
    def add_response_template(
        self, category: str, keywords: List[str], response: str
    ) -> str:
...
if __name__ == "__main__":
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        result = rag.generate_response(
            test["query"], test["intent"], test["urgency"], test.get("customer_name")
        )
